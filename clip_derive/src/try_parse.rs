...
fn impl_fields_parsing(
    ...
    } in fields
    {
        ...
        if let Some(&_) = attrs.iter().find(|attr| {
            (*attr)
                .path()
                .get_ident()
                .is_some_and(|i| i.to_string() == "try_parse")
        }) {
            gen.extend(quote! {
                {
...
pub(crate) fn impl_try_parse_macro(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let parser = match &ast.data {
        syn::Data::Struct(syn::DataStruct { fields, .. }) => {
            impl_object_initialisation(&ast.ident, fields)
        }
        syn::Data::Enum(syn::DataEnum { variants, .. }) => {
            impl_enum_initialization(&ast.ident, variants.iter())
        }
        syn::Data::Union(syn::DataUnion { union_token, .. }) => Err(syn::Error::new_spanned(
            union_token,
            "TryParse macro doesn't support union",
        )),
    }
